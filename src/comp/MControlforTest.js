...
class MControl extends Component {
	constructor(props){
		...
		this.saveResumeToDB = this.saveResumeToDB.bind(this);
	}
	...
	resetState(){
			...
			// console.log("hi, reset", this.state.usrscore, this.state.usrscore2);
			this.displayResult();
	}
	...
	fetchWords(){
			...
			fetch("http://beavver.ca-central-1.elasticbeanstalk.com/array.php",{
					method:"POST",
					body:fd
			}).then((resp)=>{
					return resp.json();
	...
	matchJob(type){
			...
			// console.log(narr, newobj, this.denominator1);
...
/*        this.refs.hello.scrollIntoView({behavior: "smooth", block: "end", inline: "nearest"});*/
	...
	fetchCourse(missing){
			...
			fetch("http://beavver.ca-central-1.elasticbeanstalk.com/courses.php",{
					method:"POST",
					body:fd
			}).then((resp)=>{
					return resp.json();
	...
	//	console.log(this.state.text1);
	var fd = new FormData();
	fd.append("resume", this.state.text1);
	fd.append("job_desc", this.state.text2);
	
	fetch("http://beavver.ca-central-1.elasticbeanstalk.com/save_resume_job.php",{
			method:"POST",
			body:fd,
