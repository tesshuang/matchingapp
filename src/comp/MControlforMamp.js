...
class MControl extends Component {
	...
	fetchWords(){
			...
			fetch("http://localhost:8888/beavverOnHost/array.php",{
					method:"POST",
					body:fd
			}).then((resp)=>{
					return resp.json();
...
/*        this.refs.hello.scrollIntoView({behavior: "smooth", block: "end", inline: "nearest"});*/
	...
	fetchCourse(missing){
			...
			fetch("http://localhost:8888/beavverOnHost/courses.php",{
					method:"POST",
					body:fd
			}).then((resp)=>{
					return resp.json();
	...
	fetch("http://localhost:8888/beavverOnHost/save_resume_job.php",{
			method:"POST",
			body:fd,
